...
const config: Configuration = merge(baseConfig, {
  ...
  output: {
    ...
    publicPath: resolve(cwd()), // 给所有的输出的添加一个公共前缀。一般是配置线上的文件路径。我这里添加了一个公共路径是/的。如果不写的话。可能刷新页面之后，html中js的相对路径就是错误的了。
    assetModuleFilename: 'images/[name][ext][query]' // 模块输出的文件名, 图片或者文件等下的路径
  },
  module: {
    rules: [
      {
